...
def plot_auc_values(auc_values_list_1, auc_values_list_2, condition, model_name, output_path):
    ...
    plt.annotate(
        f"Mean ± SD = {mean_auc_1:.3f} ± {std_auc_1:.3f}",
        xy=(70, 0.15),
        color="blue",
        fontsize=12
    )
    ...
    plt.annotate(
        f"Mean ± SD = {mean_auc_2:.3f} ± {std_auc_2:.3f}",
        xy=(70, 0.1),
        color="red",
        fontsize=12
    )
    ...
    plt.legend()
    plt.grid(alpha=0.3)
    plt.savefig(f"{output_path}AUC_plot_for_{condition}_{model_name}_with_without_ankle.svg", dpi=300, bbox_inches='tight')
    plt.show()  
